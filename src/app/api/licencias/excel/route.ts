...
// /api/dashboard/ausentismo/excel: generar un .xlsx descargable con headers
...
export async function POST(request: Request) {
  ...
  // Mismos roles que pueden entrar a /licencias (ver ROLES_PERMITIDOS en
  // licencias/page.tsx) — tampoco pueden pedir este Excel pegándole directo
  // al endpoint quienes no verían la pantalla.
  const rol = usuario.rol as RolUsuario;
  if (!["SUPERADMIN", "ADMIN", "SUPERVISOR"].includes(rol)) return new Response("Sin permiso", { status: 403 });
  ...
  const hoja = workbook.addWorksheet("Licencias");
  ...
  const nombreArchivo = `licencias_${fecha}.xlsx`;

  return new Response(buffer, {
    headers: {
      "Content-Type": "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
      "Content-Disposition": `attachment; filename="licencias.xlsx"; filename*=UTF-8''${encodeURIComponent(nombreArchivo)}`,
    },
  });
}
