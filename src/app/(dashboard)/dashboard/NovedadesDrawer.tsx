...
export default function NovedadesDrawer({ novedades, tno }: { novedades: NovedadTipoStats[]; tno: TnoStats }) {
  const [abierto, setAbierto] = useState(false);
  const total = novedades.reduce((acc, n) => acc + n.count, 0) + tno.count;
  ...
  return (
    <>
      ...
      >
        ...
        {novedades.length === 0 && tno.count === 0 ? (
          <p className="p-5 text-sm text-slate-500 text-center">Sin novedades hoy.</p>
        ) : (
          <div className="grid grid-cols-2 gap-2.5 p-5">
            {tno.count > 0 && (
              <div className="flex flex-col gap-2.5 rounded-xl border border-slate-700 p-3.5">
            ...
            {novedades.map((n) => (
              <div key={n.tipo} className="flex flex-col gap-2.5 rounded-xl border border-slate-700 p-3.5">
                <div className="flex items-center justify-between">
